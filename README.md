...
## 🧠 Deep Dive: Advanced Core Logic

### 1. Enforcement & Resource Calculation Engine

While ML is excellent for forecasting duration, physical logistics require absolute precision. ASTRAM prevents AI hallucination in resource deployment by using a **Deterministic Rule Engine**.

- **How it works:** The engine takes the `impact_score` generated by the XGBoost model and applies cascading thresholds. For example, an impact score > 80% triggers a "Critical" deployment matrix (e.g., 1 Inspector, 6 Constables, 25 Barricades, 2 Tow Trucks, 1 Ambulance). An impact score of 50% triggers a "High" matrix.
- **The Result:** Dispatchers receive reliable, mathematically backed resource allocations that scale perfectly with the severity of the incident.

### 2. Topological Graph Routing & Diversions

To calculate alternative routes, ASTRAM does not rely on external mapping APIs. It builds its own routing engine using **NetworkX**.

- **Dynamic Graph Construction:** On server startup, the backend parses historical CSV data. It extracts unique junctions as nodes and uses the **Haversine formula** to calculate the physical Earth distance (in km) between GPS coordinates to create edges (corridors). It then calculates baseline travel times based on a free-flow speed of 40 km/h.
- **Algorithmic Diversion:** When an incident occurs, the router applies a mathematical penalty multiplier to the weights of the affected corridor's edges based on the `impact_score`. It then runs **Yen's Algorithm (K-Shortest Paths)** to find the top 3 alternative routes that minimize travel time around the penalized zone, calculating exact added distances and estimated delays for the dispatch plan.
